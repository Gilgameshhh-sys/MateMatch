...
export default function Map() {
    ...
    if (!error && data) {
      const mapped = data.map((r: any) => {
        ...
        return {
          ...
          current_members:
            r.round_members?.filter((m: any) => m.status === 'confirmed')
              .length || 0,
  ...
  async function joinRound(roundId: string, type: 'open' | 'approval') {
    ...
    if (!error) {
      ...
      alert(
        type === 'open'
          ? '¡Te sumaste a la ronda!'
          : 'Solicitud enviada. El creador debe aceptarte.'
      );
    }
  }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <button style={styles.newRoundBtn} onClick={() => setShowForm(true)}>
          + Nueva ronda
        </button>
      </div>
          ...
          <Marker key={round.id} position={[round.lat, round.lng]}>
            <Popup>
              <div style={styles.popup}>
                ...
                {round.description && (
                  <p style={styles.popupDesc}>{round.description}</p>
                )}
                ...
                <div style={styles.popupMeta}>
                  <span>
                    🪑 {round.current_members}/{round.capacity} personas
                  </span>
                  <span
                    style={{
                      ...styles.typeBadge,
                ...
                <button
                  ...
                  onClick={() => joinRound(round.id, round.type)}
                >
      ...
      <div style={styles.roundsBar}>
        {loading
          ...
          : `${rounds.length} ronda${rounds.length > 1 ? 's' : ''} activa${
              rounds.length > 1 ? 's' : ''
            } cerca`}
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  modalSubtitle: { color: '#9b9bb4', fontSize: 12, margin: '0 0 4px' },
